...
public class AffinityViewWatershedTEMMax
{
...
//		}
		...
		final Compare< DoubleType > compare = ( first, second ) -> first.getRealDouble() < second.getRealDouble();

		final ArrayImg< LongType, LongArray > labels = ArrayImgs.longs( labelsDims );

		final ArrayImg< DoubleType, DoubleArray > maxs = ArrayImgs.doubles( labelsDims );
		for ( final Pair< RealComposite< DoubleType >, DoubleType > p : Views.interval( Views.pair( affsView, maxs ), maxs ) )
		{
			final RealComposite< DoubleType > s = p.getA();
			final DoubleType t = p.getB();
			t.set( -Double.MAX_VALUE );
			for ( long i = 0; i < 6; ++i )
				t.set( Double.max( s.get( i ).get(), t.get() ) );
		}

		final ArrayImg< DoubleType, DoubleArray > maxsDistTransform = ArrayImgs.doubles( labelsDims );
...
//		final TLongArrayList roots = AffinityWatershed.watershed( affsView, labels, compare, new DoubleType( Double.MAX_VALUE ), new LongType( -1 ) );
		...
		final TLongArrayList roots = Watershed.findBasinsAndFill(
				maxs,
				labels,
				new DiamondShape( 1 ),
				new LongType( -1 ),
