...
public class WatershedsExample2
{
...
//		final String url = "http://mediad.publicbroadcasting.net/p/wuwm/files/styles/medium/public/201402/LeAnn_Crowe.jpg";
		...
		final Img< FloatType > imgGS = ImageJFunctions.wrapFloat( imp2 );
		final ArrayImg< FloatType, FloatArray > gauss = ArrayImgs.floats( img.dimension( 0 ), img.dimension( 1 ) );
		Gauss3.gauss( 3.0, Views.extendBorder( imgGS ), gauss );
...
//		seedsGradientMinima( seeds, gWithNeighborhood, seed );
		...
		final DistanceDeprecated< ARGBType > distance =
				( neighborVal, currentVal, neighborPosition, seedPosition ) -> {
					...
					final int rgbaC = currentVal.get();
					final int rgbaN = neighborVal.get();
					final int dr = ARGBType.red( rgbaC ) - ARGBType.red( rgbaN );
					final int dg = ARGBType.green( rgbaC ) - ARGBType.green( rgbaN );
					final int db = ARGBType.blue( rgbaC ) - ARGBType.blue( rgbaN );
//					System.out.println( dr + " " + dg + " " + db + " " + dist );
					final double result = dr * dr + dg * dg + db * db + lambda * dist;
...
//					WatershedInterval.flood( img, seeds, new DiamondShape( 1 ), distance, new LongType( 0 ), new LongType( -1 ) );
...
//				final GenericDialog gd = new GenericDialog( "ok?" );
//				gd.showDialog();
//				new Runnable()
//				{
//
