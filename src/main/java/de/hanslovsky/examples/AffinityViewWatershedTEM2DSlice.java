...
public class AffinityViewWatershedTEM2DSlice
{
	public static void main( final String[] args )
	{
		...
		final long[] dims = new long[] { 300, 300, 2 };
		final long[] labelsDims = new long[] { 300, 300 };
		...
		final IntervalView< FloatType > dataSlice = Views.interval( Views.hyperSlice( Views.permuteCoordinates( data, perm, 3 ), 2, 0 ), new FinalInterval( dims ) );
		for ( final Pair< FloatType, DoubleType > p : Views.interval( Views.pair( dataSlice, affs ), affs ) )
...
//		BdvFunctions.show( labels, "labels" );
		...
		final BdvStackSource< ARGBType > bdv = BdvFunctions.show( new ConvertedRandomAccessibleInterval<>( Views.collapseReal( affs ), ( s, t ) -> {
			t.set( ARGBType.rgba( 255 * s.get( 0 ).get(), 255 * s.get( 1 ).get(), 0, 1.0 ) );
		}, new ARGBType() ), "affs" );
		...
		final RealRandomAccess< RealComposite< DoubleType > > rra =
				Views.interpolate( Views.addDimension( Views.extendBorder( Views.collapseReal( affs ) ) ), new NearestNeighborInterpolatorFactory<>() ).realRandomAccess();
	...
	public static class ValueDisplayListener< T extends RealType< T > > implements MouseMotionListener, OverlayRenderer
	{
		...
		public void drawOverlays( final Graphics g )
		{
			if ( probs != null )
			{
				...
				final String string = String.format( "%.4f , %.4f", this.probs.get( 0 ).getRealDouble(), this.probs.get( 1 ).getRealDouble() );
